# Minimal PowerShell launcher for B3 training (FCS->VTS cross-attention)
param(
...
$RunDirDefault = Join-Path $RunRoot "Run-B3-$TimeStr"
...
Write-Host "Starting B3 training (xattn) with:" -ForegroundColor Green
...
conda run -n dronerfa --no-capture-output python "$PSScriptRoot/train_b3.py" `
