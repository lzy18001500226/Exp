# Minimal PowerShell launcher for B4 training (Hybrid: xattn + concat + gating + aux losses)
param(
  ...
  [int]$FusionDepth = 1,
  [int]$NumHeads = 8,
  [double]$AttnDropout = 0.1,
  [double]$MlpRatio = 4.0,
  [double]$AlignLossWeight = 0.05,
  [double]$ConsistencyLossWeight = 0.02,
  [double]$ContrastiveTemperature = 0.07,
...
$RunDirDefault = Join-Path $RunRoot "Run-B4-$TimeStr"
...
Write-Host "Starting B4 training (hybrid) with:" -ForegroundColor Green
...
conda run -n dronerfa --no-capture-output python "$PSScriptRoot/train_b4.py" `
  ...
  --num_heads $NumHeads `
  --attn_dropout $AttnDropout `
  --mlp_ratio $MlpRatio `
  --align_loss_weight $AlignLossWeight `
  --consistency_loss_weight $ConsistencyLossWeight `
