...
这份文档是 `FCSDate.md` 的 VTS 版本。我们沿用同一套流水线，但把所有数据根目录、类集合和阶段策略切换到 VTS 信号：

目录结构
--------

- `Data-512NPZ/`：原始 512×512×1 VTS 光谱 NPZ，按类别编号（`0` 背景）分目录。
- `VTSLabel/`：LabelMe JSON（以及可选 PNG 掩码）。
- `SMNet/VTSData/`：转换后的检测 NPZ、健康报告以及三阶段划分。
- `SMNet/Tools/`：沿用 `convert_to_npz.py`、`split_train_val.py`、`preview_npz.py`、`analyze_label_sizes.py` 等脚本。
- `SMNet/scripts/train_npz_fcs.py`：单尺度 Anchor 训练入口（对 VTS 仍复用该脚本）。
...
python D:/Exp/SMNet/Tools/convert_to_npz.py `
  ...
  --label-root  D:/Exp/VTSLabel `
  --output-root D:/Exp/SMNet/VTSData `
  --stem-min 0 `
  --stem-max 200 `
...
1. 命令结束后检查 `SMNet/VTSData/npz_health_report.json`，确认 `golden_triplet_range` 仍位于 `[0,1]`，并关注缺失标签/空框统计。
...
4. 可用 `preview_npz.py` 抽查输出（利用保留的 `image` 字段，**注意 `--out` 必须带 `.png` 等合法扩展名**，否则 Pillow 会报 `ValueError: unknown file extension`）：
   ...
   python D:/Exp/SMNet/Tools/preview_npz.py `
     --dir D:/Exp/SMNet/VTSData/1 `
     --out D:/Exp/SMNet/VTSData/preview_cls1.png
   ```

步骤二：生成 VTS 三阶段划分（Stage 1/2/3）
---------------------------------------

VTS 我们采用“无背景 → 引入背景 → 精选类微调”的三段式；各阶段类集合如下：

1. **Stage 1（第一轮）**：仅前景 `1,3,4,5,6,8,10,11,14,15,16,17,18,19,21`（无背景）。
2. **Stage 2（第二轮）**：引入背景 0，并保持其它类 `0,1,3,4,5,6,8,10,11,14,15,16,17,18,19,21`。
3. **Stage 3（第三轮）**：精选 9 个强类别 `3,5,6,8,10,11,14,15,16` 进行长周期微调。
...
python D:/Exp/SMNet/Tools/split_train_val.py `
  D:/Exp/SMNet/VTSData `
  D:/Exp/SMNet/VTSData/splits_stage1 `
  --ratio 0.8 `
  --seed 42 `
  --label-root D:/Exp/VTSLabel `
  --label-suffix=.json `
...
python D:/Exp/SMNet/Tools/split_train_val.py `
  D:/Exp/SMNet/VTSData `
  D:/Exp/SMNet/VTSData/splits_stage2 `
  --ratio 0.8 `
  --seed 42 `
  --label-root D:/Exp/VTSLabel `
  --label-suffix=.json `
  ...
  --class-min 0 `
  --stem-max 200 `
  --keep-classes 0 1 3 4 5 6 8 10 11 14 15 16 17 18 19 21 `
  --allow-empty
```
...
python D:/Exp/SMNet/Tools/split_train_val.py `
  D:/Exp/SMNet/VTSData `
  D:/Exp/SMNet/VTSData/splits_stage3 `
  --ratio 0.8 `
  --seed 42 `
  --label-root D:/Exp/VTSLabel `
  --label-suffix=.json `
  ...
  --class-max 21 `
  --stem-max 200 `
  --keep-classes 3 5 6 8 10 11 14 15 16
```

务必核对命令行输出中 train/val 计数与类别分布；必要时 spot-check JSON。Stage 2 需要额外的背景列表（只有 class=0 的纯背景 NPZ），可用单独命令生成：

```powershell
python D:/Exp/SMNet/Tools/split_train_val.py `
  D:/Exp/SMNet/VTSData `
  D:/Exp/SMNet/VTSData/splits_stage2/background_only `
  --ratio 1.0 `
  --seed 0 `
  --label-root D:/Exp/VTSLabel `
  --label-suffix=.json `
...
Copy-Item `
  D:/Exp/SMNet/VTSData/splits_stage2/background_only/train.json `
  D:/Exp/SMNet/VTSData/splits_stage2/background.json
```
...
步骤三：训练（Stage 1 → Stage 2 → Stage 3）
-------------------------------------------
...
来自 `Tools/analyze_label_sizes.py` (针对 Stage 1/3 类集合) 的参考输入：
...
python D:/Exp/SMNet/Tools/analyze_label_sizes.py `
  ...
  --topk 5 `
  --out D:/Exp/SMNet/VTSData/anchor_stats_stage3.json
```
...
python D:/Exp/SMNet/scripts/train_npz_fcs.py `
  --train-json D:/Exp/SMNet/VTSData/splits_stage1/train.json `
  --val-json   D:/Exp/SMNet/VTSData/splits_stage1/val.json `
  --epochs 60 `
  --batch 12 `
  --hflip `
  --neg-topk-ratio 3 `
  --mixup-prob 0.0 `
  --anchor-sizes "6x5,7x6,10x5,12x6,12x6,12x7,8x19,7x23,7x23,8x22,7x29,18x16,19x20,34x22" `
  --out D:/Exp/SMNet/runs/vts_stage1 `
  --val-interval 2 `
  --per-class-ap
```
...
```powershell
python D:/Exp/SMNet/scripts/train_npz_fcs.py `
  --train-json D:/Exp/SMNet/VTSData/splits_stage2/train.json `
  --val-json   D:/Exp/SMNet/VTSData/splits_stage2/val.json `
  --epochs 40 `
  --batch 12 `
  --hflip `
  --mixup-prob 0.35 `
  ...
  --neg-topk-ratio 3 `
  --anchor-sizes "6x5,7x6,10x5,12x6,12x6,12x7,8x19,7x23,7x23,8x22,7x29,18x16,19x20,34x22" `
  --background-json D:/Exp/SMNet/VTSData/splits_stage2/background.json `
  --background-frac 0.15 `
  --out D:/Exp/SMNet/runs/vts_stage2 `
  --init-weights D:/Exp/SMNet/runs/vts_stage1/best.pth `
  --val-interval 2 `
  --per-class-ap `
  --lr 5e-4
```
...
```powershell
python D:/Exp/SMNet/scripts/train_npz_fcs.py `
  --train-json D:/Exp/SMNet/VTSData/splits_stage3/train.json `
  --val-json   D:/Exp/SMNet/VTSData/splits_stage3/val.json `
  --epochs 60 `
  --batch 12 `
  --hflip `
  --mixup-prob 0.35 `
  ...
  --neg-topk-ratio 3 `
  --anchor-sizes "6x5,7x6,10x5,12x6,12x6,12x7,8x19,7x23,7x23,8x22,7x29,18x16,19x20,34x22" `
  --out D:/Exp/SMNet/runs/vts_stage3 `
  --init-weights D:/Exp/SMNet/runs/vts_stage2/best.pth `
  --val-interval 2 `
  --per-class-ap `
  --dump-vis 10 `
  --lr 1e-4
